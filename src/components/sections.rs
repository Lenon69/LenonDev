            ...
            div class="relative z-10 container mx-auto px-4 pt-36 md:pt-28 pb-20 lg:pb-32 text-center" {
                h1 class="text-3xl lg:text-4xl font-bold tracking-tight text-brand-cyan text-shadow-strong" {
                   ...
                   div class="mt-8 max-w-3xl mx-auto text-slate-300 space-y-4 text-lg text-shadow-strong" {
                    ...
                    div class="mt-12" {
                        h2 class="text-lg font-semibold text-slate-300 mb-12 text-shadow-strong" {{"Główny stack technologiczny:"} ;
                    ...
                    @for project in projects {
                          ...
                          hx-push-url="true"
                        {
                            @if let Some(image_url) = &project.image_url {
                                img class="w-full h-48 object-cover rounded-md mb-4 group-hover:opacity-80 transition-opacity" src={ (project.thumbnail_url.as_deref().unwrap_or_else(|| project.image_url.as_deref().unwrap_or(""))) } alt=(project.title) loading="lazy" width="300" height="300";
                            }
                ...
                "@keydown.arrow-right.window"="if (modalOpen) next()"
            {
                ...
                template x-teleport="body" {
                        ...
                        "@touchend.passive"="handleTouchEnd($event)"
                    {
                        ...
                        div class="relative w-full max-w-[76rem] flex items-center justify-center" {
